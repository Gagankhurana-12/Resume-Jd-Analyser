...
- **🤖 AI-Powered Analysis**: Uses Google Gemini AI for intelligent0 resume analysis  
...
#### Recommended start command (Render)

Render runs your `startCommand` after installing dependencies. For production we recommend using Gunicorn with Uvicorn workers:

```bash
gunicorn app.main:app -k uvicorn.workers.UvicornWorker -b 0.0.0.0:$PORT -w 4 --timeout 120
```

If you saw `bash: line 1: gunicorn: command not found`, it means Gunicorn wasn't installed during the build. Fix options:

- Add `gunicorn` to `backend/requirements.txt` (we included `gunicorn==20.1.0` for you)
- Or change the start command to use `uvicorn` directly as a fallback:
